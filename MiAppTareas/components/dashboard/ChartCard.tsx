        ...
        {data.length === 0 ? (
          ...
          data.map((v, i) => (
            <View key={i} style={styles.barWrap}>
              <View style={[styles.bar, { height: `${Math.round((v / max) * 100)}%`, backgroundColor: color }]} />
            </View>
          ))
        )}
      </View>
...
const styles = StyleSheet.create({
  card: { paddingVertical: 16, paddingHorizontal: 14 },
  header: { marginBottom: 10 },
  title: { fontSize: 15, fontWeight: '700', color: colors.text.primary },
  subtitle: { fontSize: 12, color: colors.text.secondary, marginTop: 4 },
  chartRow: { flexDirection: 'row', alignItems: 'flex-end', gap: 6, height: 80 },
  barWrap: { flex: 1, justifyContent: 'flex-end' },
  bar: { width: '70%', alignSelf: 'center', borderRadius: 6 },
  noData: { flex: 1, justifyContent: 'center', alignItems: 'center', height: 60 },
  noDataText: { color: colors.text.secondary, fontSize: 13 },
});
