...
export default function ReportesExcel() {
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const response = await api.get('/configuracion/reporte-alertas-excel/', { responseType: 'blob' });
      const blob = new Blob([response.data], { type: 'text/csv;charset=utf-8;' });
      downloadBlob(blob, 'reporte_alertas.csv');
      Alert.alert('Éxito', 'Reporte Excel/CSV descargado correctamente.');
    } catch (error: any) {
      const data = error?.response?.data;
      const msg = typeof data === 'string' ? data : data?.error || 'No se pudo generar el reporte Excel.';
      ...
      <View style={styles.mainContent}>
        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent}>
          ...
          <TouchableOpacity style={styles.button} onPress={handleGenerate} disabled={loading}>
            <Text style={styles.buttonText}>{loading ? 'Generando...' : 'Generar Reporte Excel'}</Text>
          </TouchableOpacity>
        </ScrollView>
