  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        console.warn("Error cargando perfil admin:", error);
      }
    };
    loadUser();
  }, [router]);
      ...
      <View style={styles.mainContent}>
        ...
        <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <Text style={styles.userName}>{userData.nombre || "Administrador"} {userData.apellido || ""}</Text>
            <Text style={styles.userRole}>{userData.rol || "Administrador"}</Text>
          </View>
          ...
          <BottomNav items={ADMIN_BOTTOM_NAV} activeRoute="/(admin)/perfil" onPress={(route) => router.replace(route)} />
        </ScrollView>
      </View>
    </View>
  );
...
const styles = StyleSheet.create({
  safeArea: { flex: 1, flexDirection: 'row', backgroundColor: colors.background },
  mainContent: { flex: 1 },
  ...
  userRole: { fontSize: 14, color: colors.text.secondary, fontWeight: '600', textTransform: 'uppercase' },
